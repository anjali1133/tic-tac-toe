...
export const useGame = () => {
    ...
    const handleGameEnd = (data) => {
        ...
        setBoard(data.finalBoard || board);
        ...
        let resultMessage = '';
        if (data.winner) {
            const winner = players.find(p => p.userId === data.winner);
            resultMessage = `🎉 ${winner ? winner.username : 'Unknown'} wins!`;
    ...
    const connect = async (playerUsername) => {
        try {
            ...
            // Authenticate
            const session = await nakamaService.authenticate(playerUsername);
